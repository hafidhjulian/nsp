...
class Tarpel extends StatelessWidget {
  final String data2;
  Tarpel({Key key, @required this.data2});
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      debugShowCheckedModeBanner: false,
      home: Taruna(datadata: data2,),
...
class Taruna extends StatefulWidget {
  ...
  Taruna({Key key, @required this.datadata});
  @override
...
void initState() {
  ...
  Future upload(File image) async {
    ...
    var uri = Uri.parse('http://dpongs.com/APInsp/public/api/temp_nilai'); // link untuk upload foto

    var request = new http.MultipartRequest("POST", uri);

    var multipartFile = new http.MultipartFile('bukti_foto', stream, length,
        filename: path.basename(image.path));

    request.fields['no_ak'] = widget.datadata;
    request.fields['kode_pelanggaran'] = _typeAheadController.text;
    request.fields['ket_pelanggaran'] = ket.text;
    request.fields['nilai'] = nilai.text;
    request.files.add(multipartFile);
    request.fields['no_nrp'] = login.nrp1;
...
Future getData() async {
    final response =
        await http.get("http://dpongs.com/APInsp/public/api/rew_puni_1/status/1");
